...
function Blogcard({authorname, publisheddate, title, content,id}: BlogcardProps) {
  ...
  <Link to={`/blog/${id}`}>
      <div className="p-4 flex flex-col border-b pb-4 ">
       ...
       <div className="  border border-gray-300 focus:outline-none hover:bg-gray-100 text-gray-500 font-medium rounded-full  bg-gray-200 text-sm px-3 py-1">{content.length >100?Math.floor(content.length/400): 1} min read</div></div>
